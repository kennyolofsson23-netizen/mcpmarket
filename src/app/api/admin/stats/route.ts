...
export async function GET(_req: NextRequest) {
  try {
    ...
    const [pendingServers, totalUsers, totalServers, activeSubscriptions, revenueAgg, feeAgg] =
      await Promise.all([
        ...
        prisma.transaction.aggregate({ _sum: { platformFee: true }, where: { status: 'COMPLETED' } }),
      ]);
