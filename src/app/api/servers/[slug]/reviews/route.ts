...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    if (!subscription) {
      return NextResponse.json({ error: 'Subscribe to leave a review' }, { status: 403 });
    }

    const review = await prisma.review.upsert({
      where: { userId_serverId: { userId: session.user.id, serverId: server.id } },
      create: { userId: session.user.id, serverId: server.id, rating: body.rating, comment: body.comment },
