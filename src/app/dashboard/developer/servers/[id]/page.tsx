    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {server.description && (
            <p className="mt-1 line-clamp-2 text-muted-foreground">{server.description}</p>
          )}
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild variant="outline" size="sm">
            <Link href={`/dashboard/developer/servers/${server.id}/edit`}>Edit Server</Link>
          </Button>
          {!isFeatured && (
            <Button asChild size="sm" variant="outline" className="border-blue-300 text-blue-600 hover:bg-blue-50">
              <Link href={`/dashboard/developer/servers/${server.id}/featured`}>Get Featured</Link>
            </Button>
          )}
          {!server.managedHosting && (
            <Button asChild size="sm" variant="outline" className="border-green-300 text-green-600 hover:bg-green-50">
              <Link href={`/dashboard/developer/servers/${server.id}/hosting`}>Set Up Managed Hosting</Link>
            </Button>
          )}
        </div>
      </div>
