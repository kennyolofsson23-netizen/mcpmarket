...
describe("Server Detail Page", () => {
  it("renders server name, description, pricing", async () => {
    ...
    const req = new Request(
      "http://localhost/api/servers/analytics-mcp",
    ) as any;
    ...
    expect(data.description).toBe(
      "Real-time analytics powered by MCP protocol",
    );
  ...
  it("returns 404 for unknown slug", async () => {
    ...
    const req = new Request(
      "http://localhost/api/servers/not-a-real-server",
    ) as any;
    const res = await getDetail(
      req,
      makeSlugContext("not-a-real-server") as any,
    );
