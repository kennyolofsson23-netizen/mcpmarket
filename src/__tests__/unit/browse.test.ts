...
const mockFindMany = prisma.mcpServer.findMany as jest.MockedFunction<
  typeof prisma.mcpServer.findMany
>;
const mockCount = prisma.mcpServer.count as jest.MockedFunction<
  typeof prisma.mcpServer.count
>;
...
describe("GET /api/servers (browse)", () => {
  it("returns paginated list of approved servers", async () => {
    const servers = [
      makeServer(),
      makeServer({ id: "server-2", slug: "server-two" }),
    ];
  ...
  it("returns correct page and totalPages for paginated results", async () => {
    ...
    const req = new Request(
      "http://localhost/api/servers?page=2&limit=20",
    ) as any;
    const res = await GET(req);
  ...
  it("filters by category", async () => {
    ...
    const req = new Request(
      "http://localhost/api/servers?category=data",
    ) as any;
    const res = await GET(req);
  ...
  it("filters by pricing model", async () => {
    ...
    const req = new Request(
      "http://localhost/api/servers?pricingModel=FREE",
    ) as any;
    const res = await GET(req);
  ...
  it("searches by name and description", async () => {
    const matchedServers = [
      makeServer({
        name: "Postgres MCP",
        description: "A postgres data server",
      }),
    ];
    ...
    const req = new Request(
      "http://localhost/api/servers?search=postgres",
    ) as any;
    const res = await GET(req);
  ...
  it("combines multiple filters together", async () => {
    mockFindMany.mockResolvedValue([
      makeServer({ category: "data", pricingModel: "FREE" }),
    ] as any);
    mockCount.mockResolvedValue(1);

    const req = new Request(
      "http://localhost/api/servers?category=data&pricingModel=FREE&search=test",
    ) as any;
    const res = await GET(req);
