...
>;
const mockRecordApiCall = recordApiCall as jest.MockedFunction<
  typeof recordApiCall
>;
const mockGetUsageSummary = getUsageSummary as jest.MockedFunction<
  typeof getUsageSummary
>;
...
describe("POST /api/usage/record", () => {
  it("records API usage for valid API key", async () => {
    ...
    const req = new Request("http://localhost/api/usage/record", {
      ...
      headers: {
        ...
        Authorization: "Bearer mcpm_validkey",
      },
      body: JSON.stringify({
        serverId: "srv-1",
        endpoint: "/tools/list",
  ...
  it("rejects invalid API key", async () => {
    ...
    const req = new Request("http://localhost/api/usage/record", {
      ...
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer mcpm_badkey",
      },
      body: JSON.stringify({
        serverId: "srv-1",
        endpoint: "/tools/list",
...
describe("GET /api/usage/summary", () => {
  it("returns usage summary for current billing period", async () => {
    ...
    const req = new Request(
      "http://localhost/api/usage/summary?serverId=srv-1",
    ) as any;
