...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    review: { findMany: jest.fn(), count: jest.fn(), upsert: jest.fn(), create: jest.fn(), aggregate: jest.fn() },
  },
}));
...
describe('POST /api/servers/:slug/reviews', () => {
  it('creates review for subscriber', async () => {
    ...
    (prisma.mcpServer.findUnique as jest.Mock).mockResolvedValue({ id: 'srv-1', slug: 'test-server', avgRating: 0 });
    ...
    (prisma.review.upsert as jest.Mock).mockResolvedValue({
      id: 'rev-1', rating: 5, comment: 'Great!', userId: 'user-1', serverId: 'srv-1', createdAt: new Date(),
    });
    (prisma.review.aggregate as jest.Mock).mockResolvedValue({ _avg: { rating: 5 } });
    (prisma.mcpServer.update as jest.Mock).mockResolvedValue({});

    const req = new Request('http://localhost/api/servers/test-server/reviews', {
      method: 'POST',
  ...
  it('rejects review from non-subscriber', async () => {
    mockAuth.mockResolvedValue({ user: { id: 'user-1', role: 'USER', email: 'user@test.com' } } as any);
    (prisma.mcpServer.findUnique as jest.Mock).mockResolvedValue({ id: 'srv-1', slug: 'test-server' });
  ...
  it('rejects duplicate review from same user', async () => {
    mockAuth.mockResolvedValue({ user: { id: 'user-1', role: 'USER', email: 'user@test.com' } } as any);
    (prisma.mcpServer.findUnique as jest.Mock).mockResolvedValue({ id: 'srv-1', slug: 'test-server', avgRating: 4 });
    (prisma.subscription.findFirst as jest.Mock).mockResolvedValue({ id: 'sub-1', status: 'ACTIVE' });
    (prisma.review.upsert as jest.Mock).mockResolvedValue({
      id: 'rev-1', rating: 3, comment: 'Updated', userId: 'user-1', serverId: 'srv-1', createdAt: new Date(),
    });
    (prisma.review.aggregate as jest.Mock).mockResolvedValue({ _avg: { rating: 3 } });
    (prisma.mcpServer.update as jest.Mock).mockResolvedValue({});
    ...
    expect(prisma.review.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { userId_serverId: { userId: 'user-1', serverId: 'srv-1' } } }),
    );
  });
...
describe('GET /api/servers/:slug/reviews', () => {
  it('returns paginated reviews', async () => {
    ...
    (prisma.review.findMany as jest.Mock).mockResolvedValue([
      { id: 'rev-1', rating: 5, comment: 'Excellent', userId: 'user-1', serverId: 'srv-1', createdAt: new Date(), user: { id: 'user-1', name: 'Alice', image: null } },
    ]);
    (prisma.review.count as jest.Mock).mockResolvedValue(1);

    const req = new Request('http://localhost/api/servers/test-server/reviews?page=1&limit=10') as any;
