...
>;
const mockCreateStripeCustomer = createStripeCustomer as jest.MockedFunction<
...
>;
const mockServerFindUnique = prisma.mcpServer.findUnique as jest.MockedFunction<
  typeof prisma.mcpServer.findUnique
>;
const mockSubFindUnique = prisma.subscription.findUnique as jest.MockedFunction<
  typeof prisma.subscription.findUnique
>;
const mockSubUpdate = prisma.subscription.update as jest.MockedFunction<
  typeof prisma.subscription.update
>;
const mockSubCreate = prisma.subscription.create as jest.MockedFunction<
  typeof prisma.subscription.create
>;
...
describe("POST /api/subscriptions/checkout", () => {
  ...
  it("creates new Stripe customer when user has none", async () => {
    ...
    mockCreateCheckoutSession.mockResolvedValueOnce({
      url: "https://checkout.stripe.com/newsession",
    } as never);
...
describe("POST /api/subscriptions/:id/cancel", () => {
  it("cancels subscription at period end", async () => {
    ...
    mockSubUpdate.mockResolvedValueOnce({
      id: "sub-1",
      status: "CANCELED",
...
describe("POST /api/webhooks/stripe", () => {
  ...
  it("verifies Stripe webhook signature on valid request", async () => {
    mockConstructWebhookEvent.mockReturnValueOnce({
      ...
      data: {
        object: {
          metadata: { serverId: "s-1", userId: "u-1" },
