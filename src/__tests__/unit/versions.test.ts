...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    serverVersion: { findMany: jest.fn(), updateMany: jest.fn(), create: jest.fn() },
  },
}));
...
describe('POST /api/servers/:slug/versions', () => {
  it('creates new version for server owner', async () => {
    ...
    (prisma.serverVersion.updateMany as jest.Mock).mockResolvedValue({ count: 1 });
  ...
  it('sets new version as latest', async () => {
    ...
    (prisma.serverVersion.updateMany as jest.Mock).mockResolvedValue({ count: 2 });
    (prisma.serverVersion.create as jest.Mock).mockResolvedValue({
      id: 'ver-3',
      serverId: 'srv-1',
      version: '2.0.0',
...
describe('GET /api/servers/:slug/versions', () => {
  it('returns version history for server', async () => {
    ...
    (prisma.serverVersion.findMany as jest.Mock).mockResolvedValue([
      { id: 'ver-2', version: '1.1.0', isLatest: true, createdAt: new Date().toISOString() },
      { id: 'ver-1', version: '1.0.0', isLatest: false, createdAt: new Date().toISOString() },
    ]);

    const req = new Request('http://localhost/api/servers/my-server/versions') as any;
    const context = { params: { slug: 'my-server' } };
