    ...
    (prisma.mcpServer.findMany as jest.Mock).mockResolvedValue([
      {
        ...
        status: "PENDING",
        createdAt: new Date(),
        owner: { id: "dev-1", name: "Dev", email: "dev@test.com" },
      },
    ]);

    const req = new Request(
      "http://localhost/api/admin/servers/pending",
    ) as any;
...
describe("POST /api/admin/servers/:id/approve", () => {
  it("approves server and notifies developer", async () => {
    ...
    const req = new Request(
      "http://localhost/api/admin/servers/srv-1/approve",
      {
        method: "POST",
      },
    ) as any;
    ...
    (prisma.user.findMany as jest.Mock).mockResolvedValue([
      {
        ...
        role: "USER",
        createdAt: new Date(),
        _count: { servers: 0, subscriptions: 2 },
      },
      {
        id: "user-2",
        name: "Bob",
        ...
        role: "DEVELOPER",
        createdAt: new Date(),
        _count: { servers: 3, subscriptions: 0 },
      },
    ]);
